...
# from .feature_extraction import HOGFeatures, Resnet50Features, augmentation
...
class ECOMUTracker(ECOTracker):
    ...
    def local_track(self, image):
        _, out = self.track(image, self.i)
        state, score_map, test_x, scale_ind, sample_pos, sample_scales, s = out
        state = np.array(state)
        state[state < 0] = 0
        score_map = np.max(score_map, axis=2).astype(np.float64)
        score_map = cv2.resize(score_map, (19, 19))
        # update_flag = flag not in ['not_found', 'uncertain']
        # update = update_flag
        max_score = np.max(score_map, axis=(0,1))
        ...
        # learning_rate = getattr(settings, 'learning_rate', None) if hard_negative else None
        ...
        return state, score_map, logits, max_score, ap_dis.data.cpu().numpy()[0]
    ...
    def tracking(self, image):
        self.i += 1
        output_state, self.score_map, logits, score_max, dis = self.local_track(image)
        ...
        return out, [output_state, self.score_map, logits, score_max, dis]
