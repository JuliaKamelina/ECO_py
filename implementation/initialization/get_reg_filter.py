...
def get_reg_filter(sz, target_sz, reg_window_edge):
    if np.all(reg_window_edge == []):
        reg_window_edge = settings.reg_window_edge

    if (settings.use_reg_window):
        reg_window_power = settings.reg_window_power
        ...
        reg_window = (reg_window_edge - settings.reg_window_min) * \
                     (np.abs(wrs/reg_scale[0])**reg_window_power + np.abs(wcs/reg_scale[1])**reg_window_power) + \
                     settings.reg_window_min
        
        reg_window_dft = np.fft.fft(np.fft.fft(reg_window, axis=1), axis=0).astype(np.complex64) / np.prod(sz)
        reg_window_dft[np.abs(reg_window_dft) < settings.reg_sparsity_threshold * np.max(np.abs(reg_window_dft.flatten()))] = 0

        reg_window_sparse = np.real(np.fft.ifft(np.fft.ifft(reg_window_dft, axis=1), axis=0).astype(np.complex64))
        reg_window_dft[0, 0] = reg_window_dft[0, 0] - np.prod(sz) * np.min(reg_window_sparse.flatten()) + settings.reg_window_min
