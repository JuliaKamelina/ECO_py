...
from .fourier_tools import (cfft2, interpolate_dft, shift_sample,
                            ...
                            compact_fourier_coeff, sample_fs)
...
class Tracker:
    ...
    def SetScales(self, im):
        ...
        if self.nScales > 0:
            ...
            self.min_scale_factor = scale_step ** np.ceil(np.log(np.max(5 / self.img_support_sz)) /
                                                          np.log(scale_step))
            self.max_scale_factor = scale_step ** np.floor(np.log(np.min(im.shape[:2] / self.base_target_sz)) /
                                                           np.log(scale_step))
    ...
    def __init__(self, seq, im, is_color=True):
        ...
        self.ky = [np.arange(-1*int(np.ceil(sz[0] - 1)/2.0), int(np.ceil(sz[0] - 1)/2.0) + 1, dtype=np.float32)
        ...
        self.sample_space = SampleSpace(self.nSamples)
    ...
    def init_tracker(self, frame):
        ...
        if params['update_projection_matrix']:
            ...
            self.hf, self.proj_matrix = train_joint(self.hf, self.proj_matrix,
                                                    ...
                                                    self.sample_energy, self.reg_energy,
                                                    proj_energy, self.init_CG_opts)
    ...
    def track(self, frame, iter):
        ...
        if iter < params['skip_after_frame'] or self.frames_since_last_train >= params['train_gap']:
            ...
            self.hf, self.CG_state = train_filter(self.hf, self.samplesf, self.yf, self.reg_filter,
                                                  self.sample_space.prior_weights, self.sample_energy,
                                                  self.reg_energy, self.CG_opts, self.CG_state)
            self.hf_full = full_fourier_coeff(self.hf)
            self.frames_since_last_train = 0
        else:
