...
def tracker(seq):
    ...
    init_features(is_color_image, img_sample_sz, 'odd_cells')
    ...
    for i in range(0, len(filter_sz)):
        interp1, interp2 = get_interp_fourier(filter_sz[i])
    ...
    reg_filter = np.array([get_reg_filter(img_support_sz, base_target_sz, reg_win_edge)
    ...
    while True:
        ...
        if seq["frame"] == 0:  # INIT AND UPDATE TRACKER
            ...
            if params['update_projection_matrix']:
                ...
                hf, proj_matrix = train_joint(hf, proj_matrix, xlf, yf, reg_filter, sample_energy, reg_energy, proj_energy, init_CG_opts)
